...
import { FUNC_CLOSE, FUNC_GO_BACK, FUNC_SETTING, RANKING_INFO } from '../../src/constants';
...
describe('test header component', () => {
    ...
    it('test <Header /> when show ranking and not login.', () => {
        ...
        const { getByTestId } = render(
            <ThemeProvider theme={theme(false)}>
                <Header prev={RANKING_INFO} loggedIn={null} showModal={showModel} />
            </ThemeProvider>
        );

        expect(getByTestId('display_prev_link').querySelector('a').innerHTML).toEqual('<svg class="sc-bdVaJa fUuvxv" fill="#fff" width="25px" height="25px" viewBox="0 0 1024 1024" rotate="0"><path d="M352 896h128v-768h-128v768z M160 896h128v-320h-128v320z M544 896h128v-448h-128v448z M736 256v640h128v-640h-128z"></path></svg>');
        //
